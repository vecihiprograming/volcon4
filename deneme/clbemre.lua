...
if opt.checkpoints_epoch and opt.checkpoints_epoch > 0 then
  ...
  nngraph.annotateNodes() 
  model = nn.gModule({loc_prev,image_pre,image}, {loc,patch})
...
while epoch < 1 do
  ...
  -- output: {loc, image_next, visited_map_next}
	...
	--for i=1,epoch,1
	--do NewValue[i]=(((outputs[2][1] +1) * (1 - 0)) / (1 +1)) + 0
	--end
	
	print(NewValue)
	image.save('lr_patch'..tostring(epoch)..'.png',NewValue)
